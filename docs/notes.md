...
A duplicate can sit inside a list you spoke in one breath, for example "pay the gas bill, and call the vet" when the vet is already on your list. The model's first vote is on the sentence as spoken, which can go either way: it may tie the whole sentence to the vet, or call it new because the sentence as a whole is not on your list. So murmur never trusts that vote for a list. Whenever a sentence is cut into items and there is anything filed to compare against, the pieces are sent back on their own in a second, smaller request against the same items, and only the pieces that match are held, each with its own buttons. The gas bill files, the vet is held with your earlier line quoted. The log reads `[sort] compared pieces=2 of=1 held=1`. On a decision model the second request adds about a tenth of a second; on a chat model, one more round trip. If that second request cannot be answered, a sentence the first vote tied to an item is held whole as before (File it anyway lands it cut, one line per item), and a sentence it called new files cut, exactly as it would have without the check.
