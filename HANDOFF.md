...
- [ ] US-008 sleep recovery (v0.1.5, fixed 2026-09-09), two parts. Ship it: the fix is on main; publish the v0.1.5 release the same way as v0.1.2 through v0.1.4 (mac dmg from this Mac, the Windows installer from the PC) so both installed apps self-update. A signed murmur-0.1.5-arm64.dmg is already in release/ but notarization was SKIPPED (the APPLE_API_KEY / APPLE_API_KEY_ID / APPLE_API_ISSUER trio was not in the build shell), so rerun npm run dist:mac in a terminal with those set before publishing; do not publish the unnotarized one. Then verify: on the Mac, let it sleep a minute, wake it, wait about five seconds, then dictate; the first take after wake should land normally. Repeat on the PC (lid close or Start menu Sleep). If a take right at wake shows the error pill, that is the mic still coming back; the very next press should work. Report both results so the story notes and docs/troubleshooting.md can record them.
