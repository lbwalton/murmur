...
- [ ] US-008 sleep recovery (v0.1.5, fixed 2026-09-09), two parts. Ship it: the Mac half is DONE (2026-09-09): v0.1.5 released on GitHub with the notarized, stapled dmg and latest-mac.yml, so installed Macs self-update. Notarization creds now load automatically from ~/.config/apple-notary/env via .zshenv. Remaining: on the PC, pull main and run npm run dist:win, then upload the installers and latest.yml to the existing v0.1.5 release (gh release upload v0.1.5 <files>) so the PC app self-updates too. Then verify: on the Mac, let it sleep a minute, wake it, wait about five seconds, then dictate; the first take after wake should land normally. Repeat on the PC (lid close or Start menu Sleep). If a take right at wake shows the error pill, that is the mic still coming back; the very next press should work. Report both results so the story notes and docs/troubleshooting.md can record them.
