...
**Voice & model**: API key with a Test button; microphone picker with a live level check; Keep mic warm (Off, a few seconds after each dictation, or Always, which holds the mic open from launch for instant starts; the mic-in-use indicator stays lit while warm, so it is a setting); transcription model dropdown with prices inline; Smart formatting toggle with a style (Conversation, or Vibe coding to protect technical terms while dictating about code), a level (None keeps your exact words, Structure, Soft, Medium, High rewrites into polished prose), and its model dropdown; language pin; Base URL under Advanced for local or alternate endpoints.
...
**Analytics**: minutes dictated, words, average WPM, estimated API cost, and time saved versus your typing speed, computed on this machine from a local log (duration, word count, model, cost per dictation, never transcript text). Range picker, a minutes-per-day chart, a baseline typing speed setting, an off switch, and Clear data. Optional weekly, monthly, and yearly recap notifications live here too, each with its own toggle, a shared fire time, and a Test button; a recap fires once per period even if the machine was asleep at the scheduled moment.
...
## Murmur on iPhone (ios/)

The same brain, native on iOS: a SwiftUI app plus a custom Murmur keyboard, sharing the desktop's formatter rules through `shared/format-spec.json` so tuning once ships everywhere.

The keyboard detour, honestly: Apple forbids microphone access inside keyboard extensions, so no iOS keyboard can record, Murmur's included. The mic key on the Murmur keyboard hops to the Murmur app, recording starts instantly, you speak, tap stop, and your words return to the keyboard through the shared App Group store and insert at your cursor. After a take you swipe back to the app you were in; iOS offers no way for an app to send you back automatically. Wispr Flow proved this shape; Murmur does it with your own Groq key. Prefer skipping the keyboard entirely? The Action Button (or any Siri Shortcut) runs "Dictate with Murmur" and puts the transcript on your clipboard.

Setup on the phone:

1. Install from TestFlight (see `docs/ios-testflight.md` for the shipping loop).
2. Open Murmur: onboarding walks the free Groq key, the microphone, and the keyboard.
3. Settings, Murmur, Keyboards: turn on Murmur, then Allow Full Access. Full Access is what lets the keyboard read your finished dictation from Murmur's shared store; the keyboard contains no networking code and nothing you type goes anywhere.

For developers: `cd ios && xcodebuild -scheme Murmur -destination 'platform=iOS Simulator,name=iPhone 17' test` is the iOS verification gate (any installed simulator name works). Icons are generated by `npm run icons:ios`, no binary assets committed. The API key lives in the iOS Keychain; history, dictionary, corrections, and expansions never leave the device. `prd-ios.json` is the spec, same rules as the desktop one. Version bumps: `node scripts/bump-ios-version.js <version>`.
